...
// Improved Key Expansion: Ensures consistency in encryption and decryption
void expandKey(const char *key, char *expandedKey, size_t length) {
    size_t keyLen = strlen(key);
    for (size_t i = 0; i < length; i++) {
        expandedKey[i] = key[i % keyLen];  // Remove unnecessary randomness
    }
}
...
void encryptDecrypt(const char *input, char *output, const char *key) {
    ...
    char *expandedKey = (char *)malloc(inputLen);
    ...
    output[inputLen] = '\0';  // Null terminate output

    free(expandedKey);  // Free dynamically allocated memory
}
...
void encryptDecryptFile(const char *inputFile, const char *outputFile, const char *key) {
    ...
    while ((bytesRead = fread(buffer, 1, BUFFER_SIZE, in)) > 0) {
        char *keyBuffer = (char *)malloc(bytesRead);
